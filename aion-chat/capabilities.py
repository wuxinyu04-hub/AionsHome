    ...
    *,
    ...
    if is_capability_enabled("music"):
        abilities.append(
            ...
            "对方说「想听歌」「放点歌」「推荐几首」时不要只点一首——"
            "优先用 [PLAYLIST_PLAY:歌单名] 点播他已有的歌单，整单列表循环播放直到他自己关掉（像音乐 App 一样一首接一首）；"
            "真的没有合适的歌单才 [PLAYLIST_NEW] 建新的并连发 [PLAYLIST_ADD] 加 3~5 首。"
            "你能感知当前正在播放的曲目与「我们一起听过的歌」，可自然地就音乐评论或回忆。"
            "点歌时优先挑新鲜的曲目，不要重复点出「最近一起听过的歌」里已列出的，除非对方明确要求重听。"
            "不要在指令外重复歌曲信息。"
        )
        abilities.append(
            "[LIKE] — 红心当前在放的歌；[LIKE:歌曲名 歌手名] — 搜并红心一首歌，加进对方「我喜欢的音乐」。"
            "[PLAYLIST_NEW:歌单名] — 建新歌单（除非没有合适的，否则优先点播已有歌单），可附留言：[PLAYLIST_NEW:歌单名|留言]。"
            "[PLAYLIST_PLAY:歌单名] — 点播对方已有的歌单，整单列表循环播放直到对方关掉。"
