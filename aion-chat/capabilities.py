    ...
    *,
    ...
    if is_capability_enabled("music"):
        abilities.append(
            ...
            "点歌时优先挑新鲜的曲目，不要重复点出「最近一起听过的歌」里已列出的，除非用户明确要求重听。"
            "当对方想多听几首/连续听（如「想听点歌」「放点歌」「推荐几首」）时，不要只点一首——"
            "改用 [PLAYLIST_NEW] 建歌单并连发多个 [PLAYLIST_ADD] 一次加 3~5 首，"
            "对方点卡片上的「播放全部」就能整单顺序听；只有对方点名某一首时才用 [MUSIC] 单曲立即播。"
            "不要在指令外重复歌曲信息。"
        )
        abilities.append(
            "[LIKE] — 红心当前在放的歌；[LIKE:歌曲名 歌手名] — 搜并红心一首歌，加进用户「我喜欢的音乐」。"
            "[PLAYLIST_NEW:歌单名] — 建新歌单，可附一句给对方看的留言：[PLAYLIST_NEW:歌单名|留言]。"
