...
async def build_capability_prompt_items(
    ...
    *,
    ...
    if is_capability_enabled("music"):
        abilities.append(
            "[MUSIC:歌曲名 歌手名] — 对方点名要听某一首时用，立即播这一首。"
            "对方说「想听歌」「放点歌」「推荐几首」时，一口气连发 4~8 首 [MUSIC:歌曲名 歌手名]"
            "直接排进队列列表循环播放，直到他自己关掉（像音乐 App 一样一首接一首）；"
            "或者用 [DAILY_RECOMMEND] 把网易云今天的个性化推荐整单放起来；"
            "也可以 [PLAYLIST_PLAY:歌单名] 点播他已有的歌单（含红心列表「我喜欢的音乐」）。"
            "你能感知当前正在播放的曲目与「我们一起听过的歌」，可自然地就音乐评论或回忆。"
            "连发推歌时优先挑新鲜的曲目，不要重复点出「最近一起听过的歌」里已列出的，除非对方明确要求重听。"
            "不要在指令外重复歌曲信息。"
        )
        abilities.append(
            "[LIKE] — 红心当前在放的歌；[LIKE:歌曲名 歌手名] — 搜并红心一首歌，加进对方「我喜欢的音乐」。"
            "[PLAYLIST_NEW:歌单名] — 建新歌单（除非没有合适的，否则优先点播已有歌单），可附留言：[PLAYLIST_NEW:歌单名|留言]。"
            "[PLAYLIST_PLAY:歌单名] — 点播对方已有的歌单（含红心列表「我喜欢的音乐」），整单列表循环播放直到对方关掉。"
            "[PLAYLIST_ADD:歌单名] — 把当前在放的歌加进该歌单（歌单不存在会自动建）；"
            "[PLAYLIST_ADD:歌单名|歌曲名 歌手名] — 搜歌并加进指定歌单；"
            "[PLAYLIST_ADD:歌单名|歌曲名 歌手名|留言] — 加歌并附一句这首歌想对对方说的话（歌曲名留空则加当前在放的）。"
            "[PLAYLIST_REMOVE:歌单名|歌曲名 歌手名] — 从歌单移除一首，对方觉得不合口味时主动帮他打理歌单。"
            "[DAILY_RECOMMEND] — 放网易云今天的每日推荐（个性化 20 首），整单列表循环播放直到对方关掉；"
            "对方说想听歌但你没想好放什么时很合适。"
            "你建的歌单和留言会在音乐 App 里以「他的歌单」专区展示给对方，留言用心写、别模板化。"
            "你能看到对方的歌单列表（见上下文「你的歌单」），按歌单名操作即可。"
        )
